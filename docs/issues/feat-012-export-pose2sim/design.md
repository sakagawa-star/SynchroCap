...
| PySide6 | ==6.8.3 | GUI（QFileDialog, QPushButton） | 既存 |
...
def _on_export_clicked(self) -> None:
    ...
    if self._calibration_result is None or self._capture_image_size is None:
        return
    # _capture_image_size はキャリブレーション計算成功時に必ず非 None
    # （初回キャプチャ時に設定され、結果クリア時もキャプチャが残る限り維持される）
    # _current_serial: stop_live_view() で "" にクリアされるが、同時に
    # _calibration_result も None にクリアされるため、ここに到達する時点で
    # _current_serial は必ず非空文字列である
...
| エラー | 検出方法 | リカバリ | ログ |
|--------|---------|---------|------|
| ディレクトリ書き込み権限なし / ファイル書き込み失敗 | OSError catch | ステータスラベルに `Export failed: {e}` を表示。`OSError` のメッセージにファイルパスが含まれるため、失敗したファイルをユーザーが特定できる。成功済みファイルはそのまま残す | ERROR |
| ユーザーキャンセル | `directory` が空文字列 | 何もしない | — |
...
ユーザーが `QFileDialog.getExistingDirectory()` で選択するディレクトリに保存する。
...
`QFileDialog` は既存の `from PySide6.QtWidgets import (...)` ブロックに追加する。
...
- Exportボタン押下でディレクトリ選択ダイアログが開くこと
- ディレクトリ選択後にTOMLとJSONの2ファイルが生成されること
