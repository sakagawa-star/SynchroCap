...
- **受け入れ基準**: ボードが映っている場合に検出コーナーが緑色で描画され、映っていない場合はオーバーレイが消えること。検出コーナー数が6未満の場合は検出失敗として扱い、オーバーレイは表示しない。また、検出コーナーが全て一直線上に並ぶ場合（共線配置）も、キャリブレーション計算に使用できないため検出失敗として扱う（bug-009で追加）。QTimerによるフレームスキップ方式（インターバル33ms）で、検出処理中に到着したフレームは最新フレームのみ保持し、検出処理が表示を停止させないこと
- **異常系**:
