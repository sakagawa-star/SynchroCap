...
- **前提条件**: Channel Managerでチャンネル登録が完了していること
- **受け入れ基準**:
...
- **受け入れ基準**:
  - ic4 QueueSinkでフレームを取得し、BayerGR8→BGR変換（`cv2.cvtColor(frame, cv2.COLOR_BayerGR2BGR)`）後に表示される
...
- **異常系**:
  ...
  - BayerGR8変換失敗（`cv2.error`）: 該当フレームをスキップし、次のフレームを待つ
...
- **受け入れ基準**: ボードが映っている場合に検出コーナーが緑色で描画され、映っていない場合はオーバーレイが消えること。QTimerによるフレームスキップ方式（インターバル33ms）で、検出処理中に到着したフレームは最新フレームのみ保持し、検出処理が表示を停止させないこと
- **異常系**:
