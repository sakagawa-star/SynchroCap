...
def record_raw_frames(
    ...
    # Launch ffmpeg to convert BayerGR8 -> yuv420p and push into v4l2loopback.
    ffmpeg_cmd = (
        "ffmpeg -loglevel error "
        f"-f rawvideo -pix_fmt bayer_grbg8 -s {width}x{height} -framerate {fps} -i - "
        "-vf format=yuv420p "
        "-f v4l2 /dev/video1"
    )
    try:
    ...
    try:
        ...
        while datetime.now() < end_time:
            ...
            arr = buf.numpy_wrap()
            try:
                # Push raw Bayer data straight into ffmpeg (v4l2loopback output).
