...
│       ├── resourceselector.py      # IC4 リソースセレクタ
│       ├── chktimestat.py           # タイムスタンプ統計
│       └── ptp_sync_check.py        # PTP同期チェック
├── dev/
...
│   ├── BACKLOG.md            # 案件一覧
│   ├── CHANGELOG.md          # リリース履歴
│   ├── REQUIREMENTS_STANDARD.md  # 要求仕様書 記述基準
│   ├── DESIGN_STANDARD.md    # 機能設計書 記述基準
...
- **録画中のプレビュー**: 録画中もライブビュー継続（`stream_setup(sink, display)` で同時接続）。録画開始・停止時は一時的に途切れる
...
- 不具合修正時は `docs/BUGFIX_STANDARD.md` の基準に従い、修正計画を `investigation.md` に記録すること
